...
//void newCal(char *filename, char* calString, char* uidString, char* dtString);
...
ICalErrorCode createCalendar2(char* fileName, Calendar** obj, Event** evt);
...
char* eventAlarmToJSON(const Event* event);
char* eventAlarmListToJSON(const List* eventList);
char *calOpenAlarm(char *filename);
